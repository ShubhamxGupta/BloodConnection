...
const HospitalDashboard = () => {
    ...
    const totalUnits = Object.values(inventory).reduce(
        (sum, units) => sum + (Number.parseInt(units) || 0),
        0
    );
    const criticalTypes = Object.entries(inventory).filter(
        ([_, quantity]) => quantity < 10
    ).length;
    const outOfStock = Object.entries(inventory).filter(
